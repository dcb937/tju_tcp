...
void onTCPPocket(char* pkt)
{
    ...
    if (strcmp(hostname, "server") == 0) { // 自己是服务端 远端就是客户端
        ...
        // printf("sever receive, local_ip: %d,local_port: %d,remote_ip: %d, remote_port: %d\n", local_ip, local_port, remote_ip, remote_port);
    } else if (strcmp(hostname, "client") == 0) { // 自己是客户端 远端就是服务端
        local_ip = inet_network("172.17.0.2");
        remote_ip = inet_network("172.17.0.3");
        // printf("client receive, local_ip: %d,local_port: %d,remote_ip: %d, remote_port: %d\n", local_ip, local_port, remote_ip, remote_port);
    }
...
void startSimulation()
{
    ...
    BACKEND_UDPSOCKET_ID = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP); // SOCK_DGRAM : UDP
    ...
    conn.sin_family = AF_INET;
    conn.sin_addr.s_addr = htonl(INADDR_ANY); // INADDR_ANY = 0.0.0.0  即本机地址
...
int cal_hash(uint32_t local_ip, uint16_t local_port, uint32_t remote_ip, uint16_t remote_port)
{
    // 实际上肯定不是这么算的
    // 修改一下，这里原本的计算方法可能为负数。。。。。。
    int temp = ((int)local_ip + (int)local_port + (int)remote_ip + (int)remote_port) % MAX_SOCK;
    if (temp < 0)
        temp = -temp;
    return temp;
}